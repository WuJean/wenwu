...
def upload_file(file_path, api_key,user_id,  url):
    """
    上传文件或处理远程 URL。
...
def get_llm_result(inputs,api_key, user_id, workflow_url, image_url=None, upload_file_id=None, response_mode="blocking"):
    """
    从 Dify API 获取大模型的结果。
...
def upload_image_and_get_id(api_key, user_id, base_url,image_path=None, image_url=None):
    """
    上传图片并返回文件 ID 或 URL。
    """
    if image_url:  # 远程 URL
        ...
        file_id = upload_file(image_path,api_key=api_key,user_id=user_id,url=base_url+"/files/upload")
        if file_id:
            return file_id, "上传成功"
        else:
            return None, "上传失败"
...
def process_and_get_result(image_identifier, name, category, dynasty, describe, upload_type ,dify_api_token,user_id ,base_url):
    """
    将用户输入发送给大模型处理，并返回结果。
    """
